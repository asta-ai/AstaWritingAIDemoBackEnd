...
class LogRequest:
    ...
    def get_raw_id(self):
        if "RawId" not in self.header_dict.keys():
            return "default"
...
def filter_and_create_log_responses(
    ...
    for _log_item in _log_items:
        if _start_date <= _log_item.date_time < _end_date \
...
def make_statistics(_execute_parameter):
    ...
    _log_stream_requests = filter_and_create_log_requests(
        _log_items, ConstAPIName.GET_RESPONSE_STREAM,
        _execute_parameter.start_date, _execute_parameter.end_date
    )

    _stats_str += "Stream Requests: %d\n" % len(_log_stream_requests)

    _log_requests = filter_and_create_log_requests(
        _log_items, ConstAPIName.GET_RESPONSE,
        _execute_parameter.start_date, _execute_parameter.end_date
    )
    _stats_str += "Common Requests: %d\n" % len(_log_requests)

    _log_requests += _log_stream_requests
    _stats_str += "Total Requests: %d\n" % len(_log_requests)

    _log_responses = filter_and_create_log_responses(
        _log_items, ConstAPIName.GET_RESPONSE,
        _execute_parameter.start_date, _execute_parameter.end_date
    )
    _succeed, _failed = count_success_and_failed(_log_responses)

    _stats_str += "Responses (Only for Non-Stream Request): %d\n" % len(_log_responses)
...
def parse_args():
    ...
    _parser.add_argument("--start-date", default="20230120", type=str)
    _parser.add_argument("--end-date", default="none", type=str)
...
class ExecuteParameter:
    def __init__(self):
        _args = parse_args()
        self.start_date = datetime.datetime.strptime("%s 00:00:00" % _args.start_date, "%Y%m%d %H:%M:%S")
        self.end_date = None
        if _args.end_date.lower() == "none":
            self.end_date = datetime.datetime.now()
        else:
            self.end_date = datetime.datetime.strptime("%s 23:59:59" % _args.end_date, "%Y%m%d %H:%M:%S")
